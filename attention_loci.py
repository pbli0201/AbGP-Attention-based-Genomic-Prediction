...
Attention_weigts = np.dot(Attention_scores, V)
AW_colmean = np.mean(Attention_weigts, axis=0)
AW = pd.DataFrame(AW_colmean, columns=['Values']).T

Attention_scores=pd.DataFrame(Attention_scores)
Attention_weigts=pd.DataFrame(Attention_weigts)
...
for threshold in thresholds:
    ...
    # Phenotype = pd.read_pickle('adjusted_phe')
